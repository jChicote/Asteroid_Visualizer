class CelestialOrbitalMotionLogic {
    ...
    CalculateMeanAnomaly(meanAnomaly, meanMotion, time, timeOfPerihelionPassage) {
        console.log(meanAnomaly + meanMotion * (time));
        return meanAnomaly + meanMotion * (time);
    }
    ...
    CalculateOrbitalPosition(
        ...
        const distanceFromTheSun = semiMajorAxis * (1 - eccentricity * Math.cos(eccentricAnomaly));

        const positionWithinOrbitalPlane = {
            x: distanceFromTheSun * Math.cos(trueAnomaly),
            y: distanceFromTheSun * Math.sin(trueAnomaly)
        };
        ...
        return { position: positionIn3DSpace, velocity: objectVelocity };
    }
